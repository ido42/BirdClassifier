...
class ANN:
    ...
    def back_prop_m(self, result):
        last_der_loss = np.nan_to_num(np.matmul(self.layers[-1].input_vector.reshape(np.size(self.layers[-1].input_vector), 1),
                                  self.softmax_out.transpose() - result))
        self.layers[-1].grad_vector(np.nan_to_num(-self.softmax_out.transpose() + result))
        ...
        for l in range(len(self.layers) - 1, 0, -1):  # other layers
            ...
            delta = np.nan_to_num(np.matmul(self.layers[l].weight_matrix[1:], self.layers[l].grad_vect.transpose()) * der_relu)
            self.layers[l - 1].grad_vector(delta.transpose())
            der_loss =np.nan_to_num( np.matmul(self.layers[l - 1].input_vector.reshape(np.size(self.layers[l - 1].input_vector), 1),
    ...
    def dropout(self, drop_probability):
        ...
        for d in range(0, len(self.layers)):
            drop_neurons = np.random.binomial(1, 1 - drop_probability, len(self.layers[d].output_vector)).reshape(
                len(self.layers[d].output_vector), 1)
